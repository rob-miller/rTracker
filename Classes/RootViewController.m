...
- (BOOL) loadTrackerPlistFiles {
    ...
    for (file in filesToProcess) {
        ...
        if ([localFileManager removeItemAtPath:newTarget error:&err] != YES)
            DBGLog(@"Error on delete %@ to %@: %@",target, newTarget, err);
        //DBGLog(@"Unable to delete file: %@", [err localizedDescription]);
